 ...
 */
export default function PolicyRulesPanel({ isDark, hasPermission }: PolicyRulesPanelProps) {
  const canManage = hasPermission('keywords_manage');
      ...
      <CardContent className="space-y-4">
        {!canManage && (
            <div className="mb-4 p-3 bg-yellow-500/20 border border-yellow-500/30 rounded-xl text-sm text-yellow-300">
                คุณไม่มีสิทธิ์แก้ไขกฎการตรวจสอบ (โหมดดูเท่านั้น)
            </div>
        )}
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className={isDark ? 'text-gray-300 text-sm' : 'text-gray-700 text-sm'}>
            ...
            {isDirty && canManage && <span className="ml-4 text-amber-400 font-bold">มีการเปลี่ยนแปลงที่ยังไม่บันทึก</span>}
          </div>
          <div className="flex flex-wrap gap-2">
            <Button variant="outline" className="bg-transparent" onClick={handleImportClick} title="นำเข้า (.json)" disabled={!canManage}>
            ...
            <Button variant="outline" className="bg-transparent" onClick={handleReset} title="รีเซ็ตเป็นค่าตั้งต้น" disabled={!canManage}>
              <RefreshCcw className="w-4 h-4 mr-2" />
              รีเซ็ตค่าตั้งต้น
            </Button>
            <Button onClick={handleSave} title="บันทึก" disabled={!isDirty || !canManage}>
