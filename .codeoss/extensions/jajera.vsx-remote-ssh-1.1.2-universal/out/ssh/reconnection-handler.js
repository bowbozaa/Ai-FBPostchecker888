...
exports.ReconnectionHandlerImpl = void 0;
 ...
 */
class ReconnectionHandlerImpl {
    /**
     * Creates a new reconnection handler
     * @param stateManager The connection state manager
     */
    constructor(stateManager) {
        this.reconnectionCallbacks = new Map();
        this.reconnectionTimers = new Map();
        this.defaultMaxReconnectAttempts = 5;
        this.defaultReconnectBackoffFactor = 2;
        this.defaultReconnectInitialDelayMs = 1000;
        this.defaultReconnectMaxDelayMs = 60000;
        // Track active reconnection attempts to prevent duplicates
        this.activeReconnections = new Set();
     ...
     */
    classifySSHError(error, connectionId) {
    ...
    async attemptReconnection(connection) {
        ...
        try {
            ...
            const cancelPromise = vscode.window.showInformationMessage(`Attempting to reconnect to ${connection.config.host}...`, 'Cancel').then(selection => {
                if (selection === 'Cancel') {
     ...
     */
    async attemptReconnectionWithTimeout(connection, timeoutMs = defaultReconnectTimeoutMs) {
        ...
        try {
            ...
            if (error.message.includes('timed out')) {
                ...
                vscode.window.showErrorMessage(`Reconnection to ${connection.config.host} timed out after ${timeoutMs / 1000} seconds`, 'Retry', 'Cancel').then(selection => {
                    if (selection === 'Retry') {
     ...
     */
    onReconnected(connectionId, callback) {
     ...
     */
    notifyReconnected(connectionId) {
        const callbacks = this.reconnectionCallbacks.get(connectionId);
        if (callbacks) {
            // Call each callback
...
exports.ReconnectionHandlerImpl = ReconnectionHandlerImpl;
//# sourceMappingURL=reconnection-handler.js.map