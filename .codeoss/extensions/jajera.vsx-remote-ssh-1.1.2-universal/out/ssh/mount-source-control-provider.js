...
exports.MountSourceControlProviderImpl = void 0;
/**
 ...
 */
class MountSourceControlProviderImpl {
    /**
    ...
    constructor(mountManager) {
        ...
        this._repositoryInfo = new Map();
        this._disposables = [];
        this._mountManager = mountManager;
        this._errorHandler = new mount_error_handler_1.MountErrorHandler();
     ...
     */
    async initializeSourceControl(mountPoint) {
        ...
        const sourceControl = vscode.scm.createSourceControl(`git-mount-${mountPoint.id}`, `${mountPoint.name} (Git)`, mountUri);
        ...
        try {
            ...
            await this._errorHandler.handleError(error, mountPoint, `initialize Git for mount ${mountPoint.name}`);
            throw error;
        }
    ...
    async refreshSourceControl(mountId) {
        ...
        try {
            ...
            // Handle the error with user-friendly feedback
            await this._errorHandler.handleError(error, mountPoint, `refresh Git status for mount ${mountPoint.name}`);
            throw error;
        }
    }
    /**
        ...
        try {
            ...
            if (result.exitCode !== 0 && result.stderr) {
                ...
                if (result.stderr.includes('not a git repository')) {
                    await this._errorHandler.handleError(gitError, mountPoint, `execute Git command '${command}'`);
                }
            ...
            // Handle the error with user-friendly feedback
            await this._errorHandler.handleError(error, mountPoint, `execute Git command '${command}'`);
            throw error;
        }
    }
    /**
    ...
    _registerEventHandlers() {
        ...
        this._disposables.push(vscode.workspace.onDidSaveTextDocument(async (document) => {
     ...
     */
    _createResourceState(mountPoint, fileStatus) {
        // Get the mount URI
        const mountUri = this._mountManager.getMountUri(mountPoint);
        // Create the file URI
     ...
     */
    async _isGitRepository(connection, remotePath) {
        try {
     ...
     */
    async _getCurrentBranch(connection, remotePath) {
        try {
     ...
     */
    async _getRemoteUrl(connection, remotePath) {
        try {
     ...
     */
    async _getGitStatus(connection, remotePath) {
        try {
     ...
     */
    async _getConnectionForMount(mountPoint) {
...
exports.MountSourceControlProviderImpl = MountSourceControlProviderImpl;
//# sourceMappingURL=mount-source-control-provider.js.map