...
exports.RemoteFileSystemProviderImpl = void 0;
const vscode = __importStar(require("vscode"));
const path = __importStar(require("path"));
const ssh_1 = require("../interfaces/ssh");
const file_system_cache_manager_1 = require("./file-system-cache-manager");
 ...
 */
class RemoteFileSystemProviderImpl {
    constructor(connectionManager, cacheConfig = file_system_cache_manager_1.DefaultCacheConfig) {
        ...
        this.WATCH_POLL_INTERVAL_MS = 5000;
        this.connectionManager = connectionManager;
        this.cacheManager = new file_system_cache_manager_1.FileSystemCacheManager(cacheConfig);
    }
    /**
     ...
     * @returns SFTP client
     */
    async getSftpClient(connectionId) {
        ...
        if (!connection) {
            throw this.createFileSystemError('Unavailable', vscode.Uri.parse(`ssh://${connectionId}/`), 'SSH connection not found');
        }
        // Check if connection is active
        if (connection.status !== ssh_1.ConnectionStatus.Connected) {
            throw this.createFileSystemError('Unavailable', vscode.Uri.parse(`ssh://${connectionId}/`), 'SSH connection is not active');
        }
        try {
            ...
            throw this.createFileSystemError('Unavailable', vscode.Uri.parse(`ssh://${connectionId}/`), `Failed to create SFTP client: ${error.message}`);
        }
    }
    /**
    ...
    parseUri(uri) {
        ...
        if (!connectionId) {
            throw this.createFileSystemError('Unavailable', uri, 'Invalid SSH URI: missing connection ID');
        }
     ...
     */
    createFileSystemError(code, uri, message) {
        return (0, error_classifier_1.createFileSystemError)(code, uri, message);
    }
    /**
     ...
     */
    async checkPermission(sftp, remotePath, operation) {
     ...
     */
    async handleNetworkInterruption(connectionId, operation, error) {
        // Check if this is a network-related error
        const errorType = (0, error_classifier_1.classifySSHError)(error);
        ...
        const additionalNetworkError = errorMessage.includes('network') ||
        ...
        if (isNetworkError || additionalNetworkError) {
            ...
            const troubleshootingSteps = (0, error_classifier_1.getTroubleshootingSteps)(errorType);
            ...
            if (connection) {
                try {
                    ...
                    throw (0, error_classifier_1.createFileSystemError)('Unavailable', operation.uri, `Network interruption occurred during ${operation.type} operation. The operation will be retried automatically when the connection is restored.${troubleshootingText}`);
                }
                catch (reconnectError) {
                    console.error(`Failed to initiate reconnection to ${connectionId}:`, reconnectError);
                    // Throw a more user-friendly error
                    throw (0, error_classifier_1.createFileSystemError)('Unavailable', operation.uri, `Network interruption occurred during ${operation.type} operation. Failed to initiate automatic reconnection. Please try reconnecting manually.${troubleshootingText}`);
                }
            }
            else {
                // Connection not found in manager
                throw (0, error_classifier_1.createFileSystemError)('Unavailable', operation.uri, `Network interruption occurred during ${operation.type} operation. The connection was not found in the connection manager. Please reconnect manually.${troubleshootingText}`);
            }
        }
     ...
     */
    queueOperation(connectionId, operation) {
     ...
     * @param connectionId The connection ID
     */
    async executePendingOperations(connectionId) {
     ...
     */
    attributesToFileStat(attributes) {
        ...
        try {
            ...
            try {
                ...
                if (stats.type === vscode.FileType.Directory) {
                    throw this.createFileSystemError('FileNotFound', uri, `Cannot read directory as file: ${remotePath}`);
                }
            }
            catch (error) {
                ...
                throw (0, error_classifier_1.classifyAndCreateFileSystemError)(error, uri, 'read file');
            }
            // Check if we have read permission
            const hasReadPermission = await this.checkPermission(sftp, remotePath, 'read');
            if (!hasReadPermission) {
                throw this.createFileSystemError('NoPermissions', uri, `Permission denied: You don't have read permission for ${remotePath}`);
            }
            // Read the file
            let buffer;
            try {
            ...
            try {
                await this.handleNetworkInterruption(connectionId, {
                    type: 'read',
                    uri,
                    timestamp: new Date()
                }, error);
        ...
        try {
            ...
            if (fileExists) {
                ...
                if (!hasWritePermission) {
                    throw this.createFileSystemError('NoPermissions', uri, `Permission denied: You don't have write permission for ${remotePath}`);
                }
            }
            ...
            if (parentDir !== '/') {
                try {
                    ...
                    if (!hasParentWritePermission) {
                        throw this.createFileSystemError('NoPermissions', uri, `Permission denied: You don't have write permission for the parent directory ${parentDir}`);
                    }
                }
                catch (error) {
                    ...
                    throw (0, error_classifier_1.classifyAndCreateFileSystemError)(error, uri.with({ path: parentDir }), 'access parent directory');
                }
            }
            // Write the file
            ...
            throw (0, error_classifier_1.classifyAndCreateFileSystemError)(error, uri, 'write file');
        }
    }
    /**
        ...
        try {
            ...
            if (isDirectory) {
                ...
                if (options.recursive) {
                    ...
                    if (list.length > 0) {
                        throw this.createFileSystemError('NoPermissions', uri, `Cannot delete non-empty directory without recursive option: ${remotePath}`);
                    }
            ...
            // If it's not a network error, classify and create a proper file system error
            throw (0, error_classifier_1.classifyAndCreateFileSystemError)(error, uri, 'delete file or directory');
        }
    }
    /**
    ...
    async createDirectory(uri) {
        ...
        try {
            ...
            try {
                ...
                if (cachedStat) {
                    if (cachedStat.type === vscode.FileType.Directory) {
                        ...
                        throw this.createFileSystemError('FileExists', uri, `Path exists but is not a directory: ${remotePath}`);
                    }
                }
                else {
            ...
            // If it's not a network error, classify and create a proper file system error
            throw (0, error_classifier_1.classifyAndCreateFileSystemError)(error, uri, 'create directory');
        }
    }
    /**
    ...
    async readDirectory(uri) {
        ...
        try {
            ...
            try {
                ...
                if (cachedStat) {
                    if (cachedStat.type !== vscode.FileType.Directory) {
                        throw this.createFileSystemError('FileNotFound', uri, `Path is not a directory: ${remotePath}`);
                    }
                }
                else {
                    // Not in cache, check on server
            ...
            throw this.createFileSystemError('Unknown', uri, `Failed to read directory: ${error.message}`);
        }
    }
    /**
    ...
    async rename(oldUri, newUri, options) {
        ...
        if (oldParsed.connectionId !== newParsed.connectionId) {
            throw this.createFileSystemError('Unavailable', oldUri, `Cannot rename across different connections`);
        }
        ...
        try {
            ...
            // Otherwise, create a generic error
            throw this.createFileSystemError('Unknown', oldUri, `Failed to rename: ${error.message}`);
        }
    }
    /**
    ...
    async stat(uri) {
        ...
        try {
            ...
            if (error.code === 'ENOENT' || error.message?.toLowerCase().includes('not found')) {
                throw this.createFileSystemError('FileNotFound', uri, `Path not found: ${remotePath}`);
            }
            // Handle specific error types
            if (error instanceof Error) {
            ...
            // Otherwise, create a generic error
            throw this.createFileSystemError('Unknown', uri, `Failed to get file info: ${error.message}`);
        }
    }
    watch(uri, options) {
        const { connectionId, remotePath } = this.parseUri(uri);
     ...
     */
    disposeWatch(connectionId, remotePath) {
        const connectionWatches = this.watchedPaths.get(connectionId);
     ...
     * @param remotePath The remote path
     */
    scheduleWatchCheck(connectionId, remotePath) {
     ...
     * @param remotePath The remote path
     */
    async checkWatchedPath(connectionId, remotePath) {
        const connectionWatches = this.watchedPaths.get(connectionId);
        if (!connectionWatches) {
            return;
        }
        ...
        try {
            const connection = this.connectionManager.getConnection(connectionId);
            if (!connection || connection.status !== ssh_1.ConnectionStatus.Connected) {
     ...
     */
    async checkWatchedDirectory(connectionId, remotePath, watchInfo) {
        const sftp = await this.getSftpClient(connectionId);
        // List directory contents
        ...
        for (const entry of entries) {
            ...
            currentFiles.set(fullPath, {
                mtime: (entry.mtime || Date.now() / 1000) * 1000, // Convert to milliseconds
                size: entry.size || 0,
                type
            });
            ...
            if (watchInfo.options.recursive && type === vscode.FileType.Directory) {
                ...
                if (!this.watchedPaths.get(connectionId)?.has(subPath)) {
                    ...
                    this.watch(vscode.Uri.parse(`ssh://${connectionId}${subPath}`), watchInfo.options);
                }
            }
        }
     ...
     * @param watchInfo The watch info
     */
    async checkWatchedFile(connectionId, remotePath, watchInfo) {
        const sftp = await this.getSftpClient(connectionId);
        // Get file stats
        const stats = await sftp.stat(remotePath);
        const currentInfo = {
            mtime: (stats.mtime || Date.now() / 1000) * 1000, // Convert to milliseconds
            size: stats.size || 0,
            type: vscode.FileType.File
        };
...
exports.RemoteFileSystemProviderImpl = RemoteFileSystemProviderImpl;
