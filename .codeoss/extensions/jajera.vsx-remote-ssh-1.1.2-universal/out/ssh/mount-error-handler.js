...
exports.MountErrorHandler = exports.MountErrorType = void 0;
/**
 ...
 */
var MountErrorType;
(function (MountErrorType) {
    // Mount creation errors
    MountErrorType["MountCreationFailed"] = "mount_creation_failed";
    MountErrorType["MountPathInvalid"] = "mount_path_invalid";
    MountErrorType["MountAlreadyExists"] = "mount_already_exists";
    // Mount access errors
    MountErrorType["MountNotFound"] = "mount_not_found";
    MountErrorType["MountNotActive"] = "mount_not_active";
    MountErrorType["MountAccessDenied"] = "mount_access_denied";
    // Mount operation errors
    MountErrorType["MountOperationFailed"] = "mount_operation_failed";
    MountErrorType["MountConnectionLost"] = "mount_connection_lost";
    // Source control errors
    MountErrorType["GitNotInstalled"] = "git_not_installed";
    MountErrorType["NotGitRepository"] = "not_git_repository";
    MountErrorType["GitOperationFailed"] = "git_operation_failed";
    // Terminal errors
    MountErrorType["TerminalCreationFailed"] = "terminal_creation_failed";
    MountErrorType["TerminalPathResolutionFailed"] = "terminal_path_resolution_failed";
    // Unknown errors
    MountErrorType["Unknown"] = "unknown";
})(MountErrorType || (exports.MountErrorType = MountErrorType = {}));
/**
 * Mount error handler class
 * Provides specialized error handling for mount operations
 */
class MountErrorHandler {
    /**
     * Creates a new mount error handler
     */
    constructor() {
        this._sshErrorClassifier = new error_classifier_1.SSHErrorClassifier();
    }
    /**
     ...
     */
    classifyError(error, mountPoint) {
        // Default error type
        ...
        if (sshErrorType !== ssh_1.SSHErrorType.Unknown) {
     ...
     */
    async handleError(error, mountPoint, operation) {
        // Classify the error
        ...
        if (mountError.recoveryOptions && mountError.recoveryOptions.length > 0) {
            ...
            const selected = await vscode.window.showErrorMessage(message, { modal: mountError.type === MountErrorType.MountConnectionLost }, ...mountError.recoveryOptions.map(option => ({ title: option.label })));
     ...
     */
    createFileSystemError(error, uri, operation, mountPoint) {
        ...
        const fsError = new Error(`Failed to ${operation}${contextMessage}: ${error.message}${troubleshootingText}`);
     ...
     */
    _mapSSHErrorToMountError(sshErrorType) {
        switch (sshErrorType) {
            ...
            case ssh_1.SSHErrorType.FilePermissionDenied:
                return MountErrorType.MountAccessDenied;
            case ssh_1.SSHErrorType.CommandExecutionFailed:
                return MountErrorType.MountOperationFailed;
            case ssh_1.SSHErrorType.SFTPError:
                return MountErrorType.MountOperationFailed;
            case ssh_1.SSHErrorType.ConfigurationError:
     ...
     */
    _getTroubleshootingSteps(errorType) {
        switch (errorType) {
     ...
     */
    _getRecoveryOptions(errorType, mountPoint) {
                ...
                return [
                    {
                        ...
                        action: async () => {
                            if (mountPoint) {
                                ...
                                const packageManager = await vscode.window.showQuickPick(['apt-get (Debian/Ubuntu)', 'yum (CentOS/RHEL)', 'dnf (Fedora)', 'zypper (openSUSE)', 'pacman (Arch)', 'Other'], { placeHolder: 'Select the package manager on the remote server' });
                                if (!packageManager) {
                                    return;
                                }
                                ...
                                switch (packageManager) {
                                    ...
                                    default:
                                        ...
                                        vscode.window.showInformationMessage('Please install Git manually on the remote server and then try again.');
                                        return;
                                }
...
exports.MountErrorHandler = MountErrorHandler;
//# sourceMappingURL=mount-error-handler.js.map