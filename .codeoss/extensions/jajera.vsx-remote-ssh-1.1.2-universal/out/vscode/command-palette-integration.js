...
class CommandPaletteIntegration {
    constructor(extensionBridge, connectionManager, configManager, workspaceContextManager) {
     ...
     */
    async reconnectCurrentHost() {
        const activeConnections = this.connectionManager.getActiveConnections();
        if (activeConnections.length === 0) {
            this.notificationService.showNotification('No active SSH connections to reconnect', notification_service_1.NotificationLevel.Info);
            return;
        }
        ...
        try {
            ...
            this.notificationService.showNotification(`Successfully reconnected to ${connection.config.host}`, notification_service_1.NotificationLevel.Info);
        }
        catch (error) {
            // Show error with troubleshooting guidance
            this.notificationService.showTroubleshootingNotification(`Failed to reconnect to ${connection.config.host}`, [
    ...
    async closeTerminals() {
        try {
            ...
            const terminals = vscode.window.terminals.filter(terminal => terminal.name.startsWith('SSH:'));
    ...
    async openWorkspace() {
        const hosts = await this.configManager.getHosts();
        if (hosts.length === 0) {
            this.notificationService.showNotification('No SSH hosts configured', notification_service_1.NotificationLevel.Info);
            return;
        }
        const items = hosts.map(host => ({
            label: host.name,
        ...
        try {
            ...
            if (connection) {
                ...
                if (this.workspaceContextManager) {
                    await this.workspaceContextManager.createContext(connection, workspacePath, `${selected.host.name}: ${workspacePath.split('/').pop() || workspacePath}`);
                }
            }
        }
        catch (error) {
    ...
    async switchWorkspace() {
        ...
        try {
            ...
            // Save workspace context if workspace context manager is available
            if (this.workspaceContextManager) {
                await this.workspaceContextManager.createContext(connection, workspacePath, `${connection.config.host}: ${workspacePath.split('/').pop() || workspacePath}`);
            }
        }
        catch (error) {
     ...
     */
    async switchWorkspaceContext(contextId) {
        if (!this.workspaceContextManager) {
            vscode.window.showErrorMessage('Workspace context manager not available');
            return;
        }
     ...
     */
    async deleteWorkspaceContext(contextId) {
        if (!this.workspaceContextManager) {
            vscode.window.showErrorMessage('Workspace context manager not available');
            return;
        }
     ...
     */
    async renameWorkspaceContext(contextId) {
        if (!this.workspaceContextManager) {
            vscode.window.showErrorMessage('Workspace context manager not available');
            return;
        }
    ...
    async clearCache() {
        try {
            ...
            const fileCache = this.extensionBridge.fileCache;
...
- Connected since: ${connection.lastConnected.toLocaleString()}
     ...
     */
    async openWorkspaceForConnection(connectionId) {
    ...
    async showSetupGuide() {
        await this.notificationService.showSetupGuidance('SSH Remote Setup Guide', [
            {
    ...
    async clearNotifications() {
        ...
        await this.notificationService.showNotification('All notifications cleared', notification_service_1.NotificationLevel.Info);
    }
    /**
    ...
    async showPerformanceStats() {
        ...
        const { PerformanceMonitor } = await Promise.resolve().then(() => __importStar(require('../ssh/performance-monitor')));
    ...
    async showCacheStatistics() {
        try {
            // Access the file cache directly from the extension bridge
            const stats = this.extensionBridge.fileCache.getStats();
...
//# sourceMappingURL=command-palette-integration.js.map