...
exports.DetailedProgressReporter = exports.NotificationService = exports.NotificationLevel = void 0;
...
class NotificationService {
    ...
    constructor() {
        this.doNotShowAgainSettings = new Set();
        this.connectionStatusHistory = [];
     ...
     */
    async showNotification(message, level = NotificationLevel.Info, options = {}) {
        ...
        switch (level) {
            case NotificationLevel.Info:
                result = await vscode.window.showInformationMessage(message, options.modal ? { modal: true } : {}, ...buttons);
                break;
            case NotificationLevel.Warning:
                result = await vscode.window.showWarningMessage(message, options.modal ? { modal: true } : {}, ...buttons);
                break;
            case NotificationLevel.Error:
                result = await vscode.window.showErrorMessage(message, options.modal ? { modal: true } : {}, ...buttons);
                break;
        }
     ...
     */
    async showTroubleshootingNotification(message, troubleshootingSteps) {
        const viewDetails = 'View Troubleshooting Steps';
        const result = await vscode.window.showErrorMessage(message, viewDetails);
     ...
     */
    async saveDoNotShowAgainSettings() {
     ...
     */
    async showConnectionStatusNotification(host, status, level = NotificationLevel.Info) {
        // Add to history
        this.addConnectionStatusToHistory(host, status);
        // Show notification
        await this.showNotification(`SSH Connection: ${host} - ${status}`, level, { category: 'connection' });
    }
    /**
     ...
     * @param status The connection status message
     */
    addConnectionStatusToHistory(host, status) {
        // Add to history
     ...
     */
    async showConnectionStatusHistory() {
     ...
     */
    async showSetupGuidance(title, steps) {
        const viewGuide = 'View Setup Guide';
        const result = await vscode.window.showInformationMessage(title, viewGuide);
        if (result === viewGuide) {
            // Create a webview panel for interactive guidance
            const panel = vscode.window.createWebviewPanel('sshSetupGuide', 'SSH Setup Guide', vscode.ViewColumn.One, {
                enableScripts: true,
     ...
     */
    generateSetupGuidanceHtml(title, steps, webview) {
...
<body>
  ...
  <script nonce="${nonce}">
     ...
     * @returns Promise that resolves with the result of the task
     */
    async withDetailedProgress(title, steps, task) {
        return vscode.window.withProgress({
...
class DetailedProgressReporter {
     ...
     */
    reportStepProgress(message, stepProgress = 0) {
...
exports.DetailedProgressReporter = DetailedProgressReporter;
