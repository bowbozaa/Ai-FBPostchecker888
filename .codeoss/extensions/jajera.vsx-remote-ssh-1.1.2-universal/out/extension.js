...
let activeTerminals = new Map();
let savedHosts = new Map();
...
async function testSSHConnection(host, username, password) {
    try {
        ...
        return new Promise((resolve, reject) => {
            const sshProcess = (0, child_process_1.spawn)('ssh', [`${username}@${host}`, 'echo "SSH connection test successful"'], {
 ...
 */
async function openTerminalSession(host, username, password) {
    try {
 ...
 */
function disconnectFromHost(host) {
 ...
 * Reconnect to host
 */
async function reconnectToHost(host) {
 ...
 * Edit SSH host
 */
async function editSSHHost(hostId) {
...
async function importConfiguration() {
    ...
    try {
        ...
        if (config.savedHosts) {
            savedHosts.clear();
            config.savedHosts.forEach(([key, value]) => {
...
`;
                    ...
                    vscode.workspace.updateWorkspaceFolders(vscode.workspace.workspaceFolders ? vscode.workspace.workspaceFolders.length : 0, 0, { uri: mountUri, name: mountName });
        ...
        const editHostDisposable = vscode.commands.registerCommand('remote-ssh.editHost', (hostId) => {
            editSSHHost(hostId || '');
        });
        context.subscriptions.push(editHostDisposable);
        const deleteHostDisposable = vscode.commands.registerCommand('remote-ssh.deleteHost', (hostId) => {
            deleteSSHHost(hostId);
        });
        context.subscriptions.push(deleteHostDisposable);
        const setDefaultHostDisposable = vscode.commands.registerCommand('remote-ssh.setDefaultHost', (hostId) => {
        ...
        const showHostInfoDisposable = vscode.commands.registerCommand('remote-ssh.showHostInfo', (hostId) => {
            showHostInfo(hostId);
        });
        ...
        if (sshAvailable) {
            ...
            const os = require('os');
            if (os.platform() === 'win32') {
                vscode.window.showWarningMessage('SSH Extension activated but SSH not found. Please install OpenSSH for Windows or Git for Windows.', 'Install OpenSSH', 'Install Git for Windows').then(selection => {
        ...
        if (firstInstall === undefined) {
            vscode.window.showInformationMessage('VSX Remote SSH Extension has been installed. Use "Connect to Host via SSH" to get started.', 'Show Documentation').then(selection => {
