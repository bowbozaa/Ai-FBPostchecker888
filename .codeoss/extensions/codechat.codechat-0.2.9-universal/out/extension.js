...
// Local packages
// --------------
const EditorPlugin = require("./gen-nodejs/EditorPlugin");
const ttypes = require("./gen-nodejs/CodeChat_Services_types");
// Globals
// =======
...
let codechat_client_location = ttypes.CodeChatClientLocation.html;
...
function activate(context) {
    context.subscriptions.push(vscode.commands.registerCommand("extension.codeChatDeactivate", deactivate), vscode.commands.registerCommand("extension.codeChatActivate", async () => {
        ...
        if (codechat_client_location ===
            ...
            if (webview_panel !== undefined) {
                ...
                webview_panel = vscode.window.createWebviewPanel("CodeChat", "CodeChat", {
        ...
        if (thrift_connection === undefined) {
            ...
            thrift_connection.on("error", function (err) {
                console.log(`CodeChat extension: error in Thrift connection: ${err.message}`);
                was_error = true;
            ...
            thrift_connection.on("close", (hadError) => {
                ...
                if (!was_error && hadError) {
                    show_error("The connection to the CodeChat Server was closed due to a transmission error. Re-run the CodeChat extension to restart it.");
                }
...
function get_render_client(connection) {
    ...
    if (codechat_client_id === undefined) {
        ...
        thrift_client.get_client(codechat_client_location, function (err, render_client_return) {
            if (err !== null) {
                ...
                if (webview_panel === undefined) {
                    assert(codechat_client_location ===
                        ttypes.CodeChatClientLocation.browser);
    ...
    if (can_render()) {
        ...
        idle_timer = setTimeout(() => {
            if (can_render()) {
                ...
                thrift_client.start_render(vscode.window.activeTextEditor.document.getText(), vscode.window.activeTextEditor.document.fileName, codechat_client_id, vscode.window.activeTextEditor.document.isDirty, (err, start_render_return) => {
                    if (err !== null) {
...
function show_error(message) {
    if (webview_panel !== undefined) {
        ...
        webview_panel.webview.html += `<p style="white-space: pre-wrap;">${escape(message)}</p><p>See the <a href="https://codechat-system.readthedocs.io/en/latest/docs/common_problems.html" target="_blank" rel="noreferrer noopener">docs</a>.</p>`;
    }
    else {
